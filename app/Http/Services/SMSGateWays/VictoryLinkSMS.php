...
class VictoryLinkSMS
{
    ...
    {
        $params = [
            'UserName' => config('sms.VICTORY_LINK.USERNAME'),     //sender name
            'Password' => config('sms.VICTORY_LINK.PASSWORD'),
            'SMSText' => $message,                                    //text message
            'SMSLang' => $language == 'ar' ? 'A' : 'E',               // language message
            'SMSSender' => config('sms.VICTORY_LINK.SMS_SENDER'),
            'SMSReceiver' => $phone,                                  // phone number
        ];

        try {
            $smsURL = "https://smsvas.vlserv.com/KannelSending/service.asmx/SendSMS";   //api
